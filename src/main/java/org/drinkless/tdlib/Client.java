...
public final class Client {
  ...
  /**
   * Interface for handler of exceptions thrown while invoking ResultHandler. By
   * default, all such exceptions are ignored. All exceptions thrown from
   * ExceptionHandler are ignored.
   */
  ...
  public interface LogMessageHandler {
     ...
     *
     * @param verbosityLevel Log verbosity level with which the message was added
     *                       from -1 up to 1024. If 0, then TDLib will crash as soon
     *                       as the callback returns. None of the TDLib methods can
   ...
   * {@link #execute(TdApi.Function)}.
   */
  public static class ExecutionException extends Exception {
    /**
     * Original TDLib error occurred when performing one of the synchronous
     * functions.
     */
    public final TdApi.Error error;

    /**
     * @param error TDLib error occurred while performing
     *              {@link #execute(TdApi.Function)}.
     */
   ...
   *
   * @param query         Object representing a query to the TDLib.
   * @param resultHandler Result handler with onResult method which will be called
   *                      with result of the query or with TdApi.Error as
   *                      parameter. If it is null, then defaultExceptionHandler
   *                      will be called.
   */
   ...
   * requests can be executed synchronously.
   *
   * @param query Object representing a query to the TDLib.
   ...
   * @param updateExceptionHandler  Handler for exceptions thrown from
   *                                updateHandler. If it is null, exceptions will
   *                                be ignored.
   * @param defaultExceptionHandler Default handler for exceptions thrown from all
   *                                ResultHandler. If it is null, exceptions will
   *                                be ignored.
   * @return created Client
   */
  public static Client create(ResultHandler updateHandler, ExceptionHandler updateExceptionHandler,
  ...
  /**
   * Sets the handler for messages that are added to the internal TDLib log. None
   * of the TDLib methods can be called from the callback.
   *
   * @param maxVerbosityLevel The maximum verbosity level of messages for which
   *                          the callback will be called.
   * @param logMessageHandler Handler for messages that are added to the internal
   *                          TDLib log. Pass null to remove the handler.
   */
  ...
  private Client(ResultHandler updateHandler, ExceptionHandler updateExceptionHandler,
      ExceptionHandler defaultExceptionHandler) {
  ...
  private static native int nativeClientReceive(int[] clientIds, long[] eventIds, TdApi.Object[] events,
      double timeout);

  private static native TdApi.Object nativeClientExecute(TdApi.Function function);

  private static native void nativeClientSetLogMessageHandler(int maxVerbosityLevel,
